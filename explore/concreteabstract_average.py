...
class ConcreteAbstract:
    ...
    def _get_new_embeddings(self):
        ...
        #cc = self._get_classifier_capable()
        # TODO: Remove the line below in the final code.
        # This is for average embeddings
        cc = [s for s in self.abstraction_tree['SYNSET'] if not self._is_leaf(s)]
        # Start with synsets close to leaves and work our way up to more abstract hypernyms
        ...
        count = 0
        for ss in cc:
            embedding = self.abstraction_tree.loc[ss, 'EMBEDDING']
            if embedding is None:
                print('No embedding for:', ss)
                continue
    ...
    def export_embeddings(self, base_filepath):
        ...
        with open(new_embeddings_path, 'w', encoding='utf-8') as f:
    ...
    ########################################
    # Average Embeddings (Experimental)
    ########################################
    def _avg_vector(self, vecs):
        """Takes a list of vectors and returns the average."""
        z = zip(*vecs)
        m = map(lambda x: sum(x)/len(x), z)
        return list(m)
    
    def build_average_embeddings(self):
        cc = [s for s in self.abstraction_tree['SYNSET'] if not self._is_leaf(s)]
        cc.sort(key=self._dist2leaf)
        for ss in tqdm(cc):
            hypos = self.abstraction_tree.loc[ss, 'HYPONYMS']
            vecs = []
            for h in hypos:
                vecs.append(self.abstraction_tree.loc[h, 'EMBEDDING'])
            av = self._avg_vector(vecs)
            self.abstraction_tree.at[ss, 'EMBEDDING'] = av
          
    def average_embedding_count(self):
        return len(list(s for s in self.abstraction_tree['SYNSET'] if not self._is_leaf(s)))
