...
class WAC:
    ...
    def __init__(self,wac_dir,classifier_spec=(linear_model.LogisticRegression,{'penalty':'l2', 'solver':'lbfgs'}),compose_method='prod'):
        '''
    ...
    def best_word_n(self, context, gold_word, num_distractors=5):
        if gold_word not in self.trained_wac: return None
        if len(self.trained_wac) > 0:
            words = [gold_word]
            words += random.sample(list(self.trained_wac.keys()), num_distractors)
#             print(gold_word, words)
            probs = [(word, self.proba(word, context)) for word in words]
#             print(probs)
            res = max(probs, key = itemgetter(1))
            return res
        return None    
